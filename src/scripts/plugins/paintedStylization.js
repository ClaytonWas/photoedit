...
export function paintedStylization(image, parameters = {}) {
    const smallestDimension = Math.max(1, Math.min(image.width, image.height))
    const maxStrokeWidth = Math.max(1, Math.floor(smallestDimension * 0.05))   // ~5% of min dimension
    const maxStrokeLength = Math.max(5, Math.floor(smallestDimension * 0.15))  // ~15% of min dimension
...
export function vectorsInSpace(image, parameters = {}) {
    const smallestDimension = Math.max(1, Math.min(image.width, image.height))
    const maxStrokeWidth = Math.max(1, Math.floor(smallestDimension * 0.03))   // ~3% of min dimension
    const maxStrokeLength = Math.max(3, Math.floor(smallestDimension * 0.12))  // ~12% of min dimension
